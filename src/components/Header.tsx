    ...
    <header className="bg-primary text-primary-foreground shadow-md mobile-safe">
      <div className="max-w-6xl mx-auto px-2 sm:px-4 lg:px-6 py-2 sm:py-3 flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <h1 className="text-base sm:text-lg lg:text-xl font-semibold truncate">{t('app.name')}</h1>
        </div>
        {user && (
          <div className="flex items-center gap-1 sm:gap-2 lg:gap-3">
            <span className="text-xs sm:text-sm truncate max-w-[180px] sm:max-w-[200px] lg:max-w-none">
            ...
            >
              ...
              <span className="sr-only">{t('header.signOut')}</span>
            </Button>
          </div>
        )}
      </div>
